...
Ingests your docs — reads PDFs/text, splits them into chunks, converts each chunk into a vector (a list of numbers that captures meaning), and stores them in ChromaDB.
Understands your questions — when you ask something, it converts your question into the same kind of vector and finds the most semantically similar chunks from your documents.
Generates an answer — stuffs those relevant chunks as context into a prompt and sends it to a local Llama 3 model, which writes a coherent answer grounded in your actual documents.
